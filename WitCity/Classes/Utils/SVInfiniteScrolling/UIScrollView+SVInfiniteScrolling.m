...
#define kOriginX 10.0*kWidthRatio

static CGFloat const SVInfiniteScrollingViewHeight = 40;//40;
...
@property (nonatomic, strong) UIView *bottomView; //底部背景视图
@property (nonatomic, strong) UIView *animationView; //动画视图
...
- (UILabel *)label
{
    if (_label == nil) {
        ...
        _label.textColor = YLCOLOR(0x4d4d4d);
        _label.textAlignment = NSTextAlignmentCenter;
        _label.backgroundColor = YLCOLOR(0xf2f2f2);
...
-(UIView *)bottomView {
    if (!_bottomView) {
        ...
        _bottomView.backgroundColor = YLCOLOR(0xf1f1f1);
        [_bottomView addSubview:self.label];
...
- (UIView*)animationView {
    if (!_animationView) {
//        _animationView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.height/2.0, self.bounds.size.height/2.0)];
//        _animationView.centerY = self.height / 2.0;
//        if (kScreenWidth > 330) {
//            _animationView.image = [YLGIFImage imageNamed:@"底部加载@2x.gif"];
//        } else {
//            _animationView.image = [YLGIFImage imageNamed:@"底部加载@3x.gif"];
//        }
//        [self.bottomView addSubview:_animationView];
        
        UIActivityIndicatorView *activityView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
        activityView.bounds = (CGRect){0, 0, 20, 20};
        ...
        activityView.center = CGPointMake(activityView.center.x, CGRectGetHeight(self.frame) / 2);
        
        _animationView = activityView;
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        CGFloat scrollOffsetThreshold = scrollViewContentHeight-self.scrollView.bounds.size.height-self.heightToBottom;
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        CGRect viewBounds = [self.animationView bounds];
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2.0 - 20), roundf((self.bounds.size.height-viewBounds.size.height)/2));
        ...
        switch (newState) {
            ...
            case SVInfiniteScrollingStateTriggered:
                ...
                [self.bottomView addSubview:self.animationView];
                [self.label sizeToFit];
                self.animationView.left = (self.width - self.animationView.width - self.label.width-kOriginX) / 2.0;
                self.label.centerY = self.bottomView.centerY;
                self.label.left = self.animationView.right+kOriginX;
                
                if ([self.animationView isKindOfClass:[UIActivityIndicatorView class]]) {
                    [(UIActivityIndicatorView*)self.animationView startAnimating];
                }
                break;
