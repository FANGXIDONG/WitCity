...
@interface DDPwdAlertView()<UITextFieldDelegate>
@property (nonatomic,strong) UILabel     *lineLabel1;//最顶上的线
...
-(void)addDetailViews1{
    ...
    self.titlaLabel.text = @"请输入登录密码";
    self.titlaLabel.textColor = kYLColorFontBlack;
...
-(void)cancelBtnClicked{
     ...
     [ND postNotificationName:@"popToEnterprise" object:nil];
...
//    }
    ...
    if (textField.text.length<6) {
        _alertLabel.text  =@"密码不少于6位";
    }else if(textField.text.length > 20){
        _alertLabel.text  =@"密码不多于20位";
    }else{
        _alertLabel.text  =@"";
    }
    if (textField.text.length >= 6 && textField.text.length <= 20) {
        self.confitmBtn.backgroundColor = kYLColorFontBlue;
...
-(UIButton *)confitmBtn{
    if (!_confitmBtn) {
        ...
        _confitmBtn.layer.cornerRadius = 5;
        _confitmBtn.layer.masksToBounds = YES;
        [_confitmBtn setTitle:@"确认登录" forState:UIControlStateNormal];
...
-(UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setBackgroundImage:[UIImage imageNamed:@"pwd_cancel"] forState:UIControlStateNormal];
...
//        _inputTextField.keyboardType = UIKeyboardTypeNumberPad;
        ...
        _inputTextField.backgroundColor = kYLColorBakcgorund;
