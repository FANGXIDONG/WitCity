...
#pragma mark - getter
-(YLForumTableView *)tableView
{
    if(!_tableView) {
        _tableView = [[YLForumTableView alloc]initWithFrame:CGRectMake(0, 0, WINDOW_WIDTH, CONTENT_HEIGHT) style:UITableViewStylePlain];
        WS(wself);
...
{
    [YLHomeHttpUtil interactionListWithCategoryId:@"19" page:page block:^(NSDictionary *dict) {
...
- (void)pushCommentVCWithModel:(YLNewsModel *)model
{
    ...
    vc.model = model;
    vc.categoryId = @"19";
    vc.type = kYLCommetnHeaderTypeForum;
