...
@implementation DDImageItemModel
+ (instancetype)imageWithAssetURL:(NSURL *)assetURL isUploadProcess:(BOOL)isUploadProcess{
    DDImageItemModel *imageItem = [[DDImageItemModel alloc] init];
    ...
    void (^selectAsset)(ALAsset *) = ^(ALAsset *asset){
        if (asset) {
            UIImage *highQualityImage = [DDImageCompressHelper fullScreenImageALAsset:asset];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                imageItem.image = [DDImageCompressHelper compressedImageToLimitSizeOfKB:100 image:highQualityImage];;
                imageItem.thumbnailImage = thumbnailImage;
                
                if (isUploadProcess) {
                    //上传到沙盒  成功上传后记得删除对应
                    [DDFileManager writeUploadDataWithName:imageItem.photoName andImage:imageItem.image];
                }
            });
        }
    };
    ...
    DDWeakSelf(assetsLibrary);
    [assetsLibrary assetForURL:assetURL resultBlock:^(ALAsset *asset) {
        if (asset) {
            selectAsset(asset);
        }else{
            DDStrongSelf(assetsLibrary);
...
//获得图片基本信息
+(void)imageInfoWithALAsset:(ALAsset *)asset imageItem:(DDImageItemModel *)imageItem
{
    //拍照时间
    NSDate * nsALAssetPropertyDate = [ asset valueForProperty:ALAssetPropertyDate ];
    if (nsALAssetPropertyDate!=nil) {
        imageItem.photoTime=[DDDateTimeHelper htcTimeToLocationStr:nsALAssetPropertyDate];
    }
